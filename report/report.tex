...
This functions is responsible for signaling the beginning of the file transmission process to the receiver. This also catches any errors that happened in the transmission of the \textit{START} packet.
First we \textbf{stat} the file to be transmitted in order to get its size. Then a \textit{START} packet is constructed and sent to the receiver system. Finally we return 1 on success or -1 on failure.
...
Varying the frame error ratio resulted in a steep decline in efficiency as the FER increases. This can be due to the fact that in our tests, when we wanted to fall inside the "signal error" area, we outright reject an entire frame, which requires us to wait for \textit{timeout} seconds before re-transmitting the previous frame.

\subsection{Varying the Cable Length}

Varying the cable length resulted in a near-linear decrease in transmission efficiency.

This makes sense because the transmission speed of data packets/frames is constant throughout the cable, so the efficiency is proportional to the length.

FIXME: mudar isto

\subsection{Varying the Packet Size}

Varying the packet size resulted in a functions that is close to \textit{(-1/x) + 1}, showing that the optimal packet size is between 1024 and 2048: any values bigger do not incur any performance gain.
